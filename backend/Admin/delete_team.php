...
class AdminDeleteUser {
    ...
    public function controllerDeleteTeamMembers($id) {
        try {
            $result = $this->deleteModel->deleteTeamById($id);
            if ($result == true) {
    ...
    public function controllerDeleteAllTeamMembers() {
        try {
            $result = $this->deleteModel->deleteAllTeams();
...
if ($id) {
    ...
    $manager->controllerDeleteTeamMembers($id);
} else {
    echo json_encode(['success' => false, 'message' => 'No user ID provided']);
}
// $id = 15;
// $admin->controllerDeleteUser($id); // Example usage, replace with actual user ID